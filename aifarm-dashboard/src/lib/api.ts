...
class APIClient {
  ...
  async getDORequests(params?: { status?: string; limit?: number }) {
    ...
    return this.request<{ requests: any[] }>(`/api/do-requests${query ? `?${query}` : ''}`);
  }
  ...
  async createDORequest(data: {
    ...
    return this.request<any>('/api/do-requests', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async updateDORequest(requestId: string, status: string) {
    return this.request<any>(`/api/do-requests/${requestId}?status=${status}`, {
      method: 'PATCH',
    });
  }

  async cancelDORequest(requestId: string) {
    return this.updateDORequest(requestId, 'cancelled');
  }
