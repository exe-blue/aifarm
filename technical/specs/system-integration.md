...
INSERT INTO wormhole_config VALUES
  (1, 'echo',   'Type α: 같은 모델끼리의 동기화',    5,    0.90, TRUE,  FALSE, TRUE),
  (2, 'bridge', 'Type β: 다른 모델 간의 기이한 일치', 10,   0.85, FALSE, FALSE, TRUE),
  (3, 'loop',   'Type γ: 시간차를 둔 자기 공명',    86400, 0.90, NULL,  TRUE,  TRUE);
```
...
FUNCTION CALCULATE_CONFIDENCE(cluster):
    ...
    time_bonus = 0.1 * (1 - cluster.time_spread / MAX_TIME_WINDOW)  -- 시간차 작을수록 +
