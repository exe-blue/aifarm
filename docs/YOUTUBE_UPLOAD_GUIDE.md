...
**5대 PC 노드** × **120대 안드로이드** = **600대 디바이스**로 YouTube 영상을 시청하고, 결과를 집계하는 시스템입니다.
...
Gateway (중앙 관제)
    ↓
PC_01 (120대) + PC_02 (120대) + PC_03 (120대) + PC_04 (120대) + PC_05 (120대)
    ↓
총 600대 안드로이드 디바이스
```
...
**목적**: 600대 디바이스별 작업 할당 및 결과 저장 (5대 PC 노드 구조)

| 컬럼 | 타입 | 설명 |
|------|------|------|
| task_id | UUID | 기본키 |
| video_id | UUID | 영상 FK |
| device_serial | VARCHAR | ADB 시리얼 번호 (예: PC_01_001) |
| **pc_id** | VARCHAR | **PC 노드 ID (PC_01 ~ PC_05)** |
| **pc_device_index** | INTEGER | **PC 내 인덱스 (0~119)** |
